...
########################maxF##############################
    ...
    def show(self,all_num=None):
        # assert self.num == self.idx
        self.precision =  self.precision#[:self.idx]
        self.recall =  self.recall#[:self.idx]
        self.meanF =  self.meanF#[:self.idx]
        # if all_num is None:
        if all_num is None:
            precision = self.precision.mean(axis=0)
            recall = self.recall.mean(axis=0)
        else:
            precision = self.precision.sum(axis=0)/all_num
            recall = self.recall.sum(axis=0)/all_num
...
class cal_mae(object):
    ...
    def show(self,all_num=None):
        if all_num is None:
            return np.mean(self.prediction)
        return np.sum(self.prediction)/all_num
...
class cal_wfm(object):
    ...
    def show(self,all_num=None):
        ...
        return np.sum(self.scores_list)/all_num
